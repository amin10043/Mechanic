...
import com.project.mechanic.server.SavingProvinceCity;
...
public class CreateShopAdvisorExecuter extends Fragment
		implements AsyncInterface, SaveAsyncInterface, ProvinceCityInterface {
	...
	int serverProvinceId = -1;
	int serverCityId = -1;

	public CreateShopAdvisorExecuter(int mainObjectId, int objectTypeId, int cityId) {
	...
	private void imagesClick() {
		...
		btnHeader.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				getActivity().startActivityFromFragment(CreateShopAdvisorExecuter.this, i, headerGallery);
		...
		btnProfile.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				getActivity().startActivityFromFragment(CreateShopAdvisorExecuter.this, i, RESULT_LOAD_IMAGE);
			}
		});
		...
		btnFooter.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				getActivity().startActivityFromFragment(CreateShopAdvisorExecuter.this, i, footerGallery);
			}
		});
	}
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (nameValue.equals("")) {
					...
					date.delegate = CreateShopAdvisorExecuter.this;
					date.execute("");
	...
	public void processFinish(String output) {
		if (util.checkError(output) == false) {
			...
			try {
				...
				agencyId = Integer.valueOf(output);

				DBAdapter.open();

				DBAdapter.createNewShopAdvisorExecuter(agencyId, nameValue, phoneValue, emailValue, faxValue,
						descriptionValue, catalog, price, pdf, addressValue, mobileValue, faceBook, instagram, linkedIn,
						google, websiteValue, twiiter, currentUser.getId(), mainObjectId, objectTypeId, serverDate);

				DBAdapter.close();

				SavingImage3Picture savingImage = new SavingImage3Picture(getActivity());
				savingImage.delegate = CreateShopAdvisorExecuter.this;
				...
				saving.delegate = CreateShopAdvisorExecuter.this;
				...
				params.put("ObjectBrandTypeId", String.valueOf(StaticValues.NoAgencyNoService));
		...
		if (util.checkError(output) == false) {
			...
			if (util.checkError(output) == false) {
				...
				try {
					...
					if (byteHeader != null) {
						util.CreateFile(byteHeader, agencyId, "Mechanical", "Profile", "header", "Object");
						DBAdapter.updateObjectImage1ServerDate(agencyId, output);

					}
					if (byteProfil != null) {
						util.CreateFile(byteProfil, agencyId, "Mechanical", "Profile", "profile", "Object");
						DBAdapter.updateObjectImage2ServerDate(agencyId, output);

					}
					if (byteFooter != null) {
						util.CreateFile(byteFooter, agencyId, "Mechanical", "Profile", "footer", "Object");
						DBAdapter.updateObjectImage3ServerDate(agencyId, output);

					}

					City city = DBAdapter.getCityById(cityId);
					Province province = DBAdapter.getProvinceById(city.getProvinceId());
					provinceId = province.getId();

					DBAdapter.close();

					params = new LinkedHashMap<String, String>();
					SavingProvinceCity saving = new SavingProvinceCity(getActivity());
					saving.delegate = CreateShopAdvisorExecuter.this;

					params.put("TableName", "ObjectInProvince");

					params.put("ObjectId", String.valueOf(agencyId));
	...
	@Override
	public void ResultProvinceCity(String output) {
		...
		if (util.checkError(output) == false) {
			...
			if (isProvince == true) {
				...
				try {
					...
					serverProvinceId = Integer.valueOf(output);

					DBAdapter.open();
					DBAdapter.insertObjectInProvince(serverProvinceId, agencyId, provinceId, serverDate);
					DBAdapter.close();
				...
				try {
					...
					serverCityId = Integer.valueOf(output);

					DBAdapter.open();
					DBAdapter.insertObjectInCity(serverCityId, agencyId, cityId, serverDate);
					DBAdapter.close();
					...
					bundle.putString("Id", String.valueOf(agencyId));
